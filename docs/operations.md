...
`copper-move-search-flow` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies search behavior through capacity fixtures, with allocation and spill reports and bounded memory input sets.
...
The score is deterministic. The current threshold is 159. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 4.
